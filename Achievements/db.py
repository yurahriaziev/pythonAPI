...
class DB:
    def __init__(self):
        ...
        self.cur.execute('CREATE TABLE IF NOT EXISTS players (username, game_list, password)')
        self.cur.execute('CREATE TABLE IF NOT EXISTS achievements (title, game, player_id)')
        self.conn.commit()

    def add_player(self, user, games, password):
        task = 'INSERT INTO players (username, game_list, password) VALUES (?, ?, ?)'
        if len(games) > 1:
            plr_games = ''
            for game in games:
                plr_games += f'{game} '
            ...
            plr_games = games[0]
        self.cur.execute(task, (user, plr_games, password,))
        self.conn.commit()
